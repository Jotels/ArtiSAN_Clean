...
def sync_params(module: torch.nn.modules.Module):
    ...
    for p in module.parameters():
        p_numpy = p.data.cpu().numpy()  # numpy view of tensor data (shares the same underlying storage)
        mpi_broadcast(p_numpy)
        if p.data.is_cuda:
            p.data = torch.from_numpy(p_numpy).to(p.data.device)
...
def mpi_avg_grads(module: torch.nn.modules.Module):
    ...
    for p in module.parameters():
        ...
        if p.grad.is_cuda:
            p.grad = torch.from_numpy(p_grad_numpy).to(p.grad.device)
