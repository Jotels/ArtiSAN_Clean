...
def get_all_formulas():
    ...
    reduced_formula_list = ['Al2Au', 'CuAu', 'Cu3Au', 'Al2Pd', 'Ag3Pt', 'Al2Pt']
...
def get_systems(config):
    ...
    compositions, fractions = get_all_formulas()
    eval_alloys = []
...
def evaluate_model(agent: ArtiSAN,
    ...
    path_to_traj_folder = config["Fig5_Traj_Dir"] + f"/{alloy_type}/{composition_type}"

    check_create_dir(path_to_traj_folder)

    wandb.init(config=config, project=config["Fig5_Project_Name"], group=f"{alloy_type}")
    ...
    if __name__ == '__main__':
        stream = open("../ArtiSAN/src/hyperparameter.yaml", 'r')
