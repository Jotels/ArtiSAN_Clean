...
def get_all_formulas():
    ...
    reduced_formula_list = ['NiCuPdPtAu', 'NiCuPdAgPt', 'CuPdAgPtAu']
...
def evaluate_model(agent: ArtiSAN,
    ...
    path_to_traj_folder = config["Fig6_Traj_Dir"] + f"/{alloy_type}/{composition_type}"

    check_create_dir(path_to_traj_folder)

    wandb.init(config=config, project=config["Fig6_Project_Name"], group=f"{alloy_type}")
